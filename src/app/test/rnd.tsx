...
const Rnd: React.FC<RndProps> = ({
  ...
  const [startMouse, setStartMouse] = useState<{ x: number; y: number } | null>(
    null,
  );
      ...
      >
        <Rnd
          ...
          onDragStart={(xPercent, yPercent, xPx, yPx) => {
            setDragInfo(
              `Drag Start: ${xPercent.toFixed(2)}% (${xPx.toFixed(0)}px), ${yPercent.toFixed(2)}% (${yPx.toFixed(0)}px)`,
            );
          }}
          onDrag={(xPercent, yPercent, xPx, yPx) => {
            setDragInfo(
              `Dragging: ${xPercent.toFixed(2)}% (${xPx.toFixed(0)}px), ${yPercent.toFixed(2)}% (${yPx.toFixed(0)}px)`,
            );
          }}
          onDragStop={(xPercent, yPercent, xPx, yPx) => {
            setDragInfo(
              `Drag Stop: ${xPercent.toFixed(2)}% (${xPx.toFixed(0)}px), ${yPercent.toFixed(2)}% (${yPx.toFixed(0)}px)`,
            );
          }}
          onResizeStart={(
            widthPercent,
